...
async def _handle_checkin_submission(
    ...
    if user_email_l in CIERRE_CAJA_USERS:
        ...
        if any_filled or caja_notas:
            try:
                _cierre_guardado = activity_state.set_cierre_caja(
                ...
                cierre_email_result = {
                    "ok": True, "to": [], "cc": [],
                    "total": _cierre_guardado.get("total"),
                }
    ...
    try:
        if marcadas_daily > 0 or marcadas_weekly > 0:
            ...
            if cierre_email_result and cierre_email_result.get("ok"):
                _tot = cierre_email_result.get("total")
                extra_msg = (
                    f"\n\n💵 Cierre de caja registrado"
                    + (f": **${_tot:,.2f}** en caja" if _tot is not None else "")
                    + " — va en el resumen de gerencia de las 6:30 PM."
                )
            elif (
                user_email_l in CIERRE_CAJA_USERS
                and not activity_state.get_cierre_caja(user_email, today_iso)
            ):
                # Incidente 2026-07-16: el submit llegó con la sección de caja
                # vacía y el bot confirmó "✅" sin avisar — el consolidado dijo
                # "sin marcar" y la asistente juraba que lo había llenado.
                extra_msg = (
                    "\n\n⚠️ **OJO: no llenaste el cierre de caja de hoy.** "
                    "El reporte de gerencia va a decir 'sin marcar'. "
                    "Volvé al formulario de arriba, completá las denominaciones "
                    "y tocá GUARDAR otra vez (las actividades ya quedaron — "
                    "solo se agrega el cierre)."
                )
            await context.send_activity(
...
# Extraída a cobranzas_sync.py (gate F4: teams_bot sigue bajando).
# Re-export para compat de tests/admin que referencian teams_bot.*
from cobranzas_sync import (  # noqa: E402
...
async def send_cierre_caja_recordatorio_job() -> None:
    """18:05 Lun-Vie EC: si un asistente de caja NO marcó el cierre de HOY, se
    lo recuerda por Teams ANTES del consolidado de las 18:30 (incidente
    2026-07-16: el submit llegó sin la sección de caja, nadie se enteró y el
    reporte a gerencia salió con 'sin marcar')."""
    hoy_iso = activity_state._today().isoformat()
    refs = _load_refs().get("activities", {})
    enviados = 0
    for email in sorted(CIERRE_CAJA_USERS):
        try:
            if activity_state.get_cierre_caja(email, hoy_iso):
                continue  # ya marcó — nada que recordar
        except Exception:
            logger.exception("recordatorio cierre: no pude leer cierre de %s", email)
            continue
        ref_dict = refs.get(email)
        if not ref_dict:
            logger.warning("recordatorio cierre: %s sin ref del Activities Bot", email)
            continue
        try:
            ref = ConversationReference().deserialize(ref_dict)
            ...
            async def cb(turn_context: TurnContext) -> None:
                await turn_context.send_activity(
                    "🔔 **Falta tu cierre de caja de hoy.** El resumen a "
                    "gerencia sale a las 6:30 PM — tipea `/checkin`, completá "
                    "las denominaciones de la caja y tocá GUARDAR (si ya "
                    "marcaste tus actividades, solo se agrega el cierre)."
                )
            ...
            enviados += 1
            logger.info("Recordatorio de cierre de caja enviado a %s", email)
        except Exception as e:
            logger.exception("Falló recordatorio de cierre a %s: %s", email, e)
    logger.info("send_cierre_caja_recordatorio: %d enviados", enviados)
...
async def _job_cierre_caja_recordatorio() -> None:
    await _reliable_job(
        "cierre_caja_recordatorio", send_cierre_caja_recordatorio_job,
        ledger_key="cierre_caja_recordatorio",
    )
...
def _schedule_jobs() -> None:
    ...
    if _mod("activities"):
        ...
        # 2026-07-17: recordatorio de cierre de caja faltante, antes del
        # consolidado de 18:30 (incidente del 16/07).
        scheduler.add_job(
            _job_cierre_caja_recordatorio,
            _cron_for("cierre_caja_recordatorio"),
            id="cierre_caja_recordatorio",
            replace_existing=True,
        )
...
def _catchup_specs() -> list[tuple[str, Any, Any]]:
    ...
    if _mod("activities"):
        specs += [
            ...
            ("cierre_caja_recordatorio", _job_cierre_caja_recordatorio,
             _due_after("cierre_caja_recordatorio")),
