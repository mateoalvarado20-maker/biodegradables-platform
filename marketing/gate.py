...
def deterministic_checks(
    ...
    """Violaciones objetivas. Lista vacía = pasa a revisión de marca."""
    problems: list[str] = []
    ...
    texto = _package_texts(package).lower()
    for claim in hard_rules.get("claims_prohibidos", []):
        if claim.lower() in texto:
...
def review_package(
    ...
    *,
    ...
    """Devuelve el package en `qa_approved` o `qa_rejected` (nuevo objeto)."""
        ...
        if problems:
            ...
            for _attempt in range(2):
                ...
                try:
                    ...
                    messages.append(
                        {"role": "user", "content": f"No validó: {exc}. Solo el JSON."}
                    )
            if data is None:
